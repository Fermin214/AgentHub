...
function PromptCard({ prompt, onPreview, onEdit, onFavorite, onDelete }: { prompt: Prompt; onPreview: () => void; onEdit: () => void; onFavorite: () => void; onDelete: () => void }) {
  const t = useT();
  return <article className="prompt-card"><div className="prompt-card__top"><span /><div className="prompt-card__actions"><IconButton label={prompt.favorite ? t('common.unfavorite') : t('common.favorite')} className={prompt.favorite ? 'icon-button--favorite' : ''} onClick={onFavorite}><Star size={16} fill={prompt.favorite ? 'currentColor' : 'none'} /></IconButton><IconButton label={t('prompts.editLabel')} onClick={onEdit}><Pencil size={15} /></IconButton><IconButton label={t('prompts.deleteLabel')} onClick={onDelete}><Trash2 size={15} /></IconButton></div></div><button className="prompt-card__copy" onClick={onPreview} aria-label={t('prompts.preview.label',{title:prompt.title})}><h3>{prompt.title}</h3>{prompt.purpose && <div className="prompt-card__purpose">{prompt.purpose}</div>}<p>{prompt.body}</p></button><div className="prompt-card__bottom"><div className="tag-list">{prompt.tags.map((tag) => <span key={tag}>#{tag}</span>)}</div><time dateTime={prompt.updatedAt}>{t.relative(prompt.updatedAt)}</time></div></article>;
}
