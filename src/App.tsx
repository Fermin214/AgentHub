...
function Workspace({ lang, onLang }: { lang: Lang; onLang: (lang: Lang) => void }) {
  ...
  // Applied when a save returns the authoritative records, so the UI reflects the
  // committed result immediately instead of waiting for the next full snapshot.
  const applySkills = useCallback((skills: Skill[]) => setSnapshot(current => current ? { ...current, skills } : current), []);
