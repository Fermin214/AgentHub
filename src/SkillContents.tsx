...
export function SkillContents({skill,deployments}:{skill:Skill;deployments:SkillDeployment[]}) {
  ...
  return <section className="toolkit-reader"><h3>{t('contents.title')}</h3><label className="field"><span>{t('contents.location')}</span><select value={deploymentId} onChange={e=>setDeployment(e.target.value)}><option value="">{t('contents.library')}</option>{deployments.map(d=><option value={d.id} key={d.id}>{agentName(d.agent)} · {displayPath(d.path)}</option>)}</select></label>{error&&<p role="alert" className="text-error">{t.backend(error)}</p>}<div className="toolkit-reader__layout"><nav aria-label={t('contents.files')}>{files.map(f=><button key={f.path} disabled={f.previewable===false} className={f.path===path?'active':''} onClick={()=>setPath(f.path)}>{f.path}</button>)}</nav><div className="toolkit-reader__content">{busy?<p role="status">{t('contents.reading')}</p>:<><h4>{path}</h4><pre><code>{content}</code></pre></>}</div></div></section>;
}
