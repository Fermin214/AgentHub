...
The Release profile supports optional adapters for authorized runtime isolation, real browser download/startup, and packaged native checks; see [acceptance automation](docs/acceptance-automation.md) for prerequisites and scope. Use the [manual Windows runbook](docs/acceptance-manual.md) for remaining checks. Full release acceptance combines the automated report with those operator records; supplemental header evidence does not complete the profile. Do not publish GitHub Releases automatically.
